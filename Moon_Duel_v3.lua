...
-- COMPACT MODE button — collapses the panel content into a thin status
-- strip (ping + Auto Steal on/off), independent from the close/hide-to-moon
-- animation. Purely additive: only toggles Visible + tweens Size, nothing
-- else is touched, so it cannot conflict with hideGui/showGui or UI Scale.
local compactBtn = Instance.new("TextButton", titleBar)
compactBtn.Size = UDim2.new(0,22,0,22); compactBtn.Position = UDim2.new(1,-82,0.5,-11)
compactBtn.BackgroundColor3 = Color3.fromRGB(0,0,0); compactBtn.BorderSizePixel = 0
compactBtn.Text = "▾"; compactBtn.TextColor3 = C_WHITE
compactBtn.Font = Enum.Font.GothamBlack; compactBtn.TextSize = 13
compactBtn.ZIndex = 7; addCorner(compactBtn, 8); addLivingStroke(compactBtn, 1)
compactBtn.MouseEnter:Connect(function() TweenService:Create(compactBtn,TweenInfo.new(0.1),{TextColor3=C_MOON2}):Play() end)
compactBtn.MouseLeave:Connect(function() TweenService:Create(compactBtn,TweenInfo.new(0.1),{TextColor3=C_WHITE}):Play() end)
...
compactSteal.Size = UDim2.new(0,140,1,0); compactSteal.Position = UDim2.new(1,-154,0,0)
...
local function setCompactMode(on)
	...
	TweenService:Create(mainOuter, TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
		{Size = UDim2.new(0,WIN_W,0, on and COMPACT_H or WIN_H)}):Play()
end
compactBtn.MouseButton1Click:Connect(function() setCompactMode(not _compactMode) end)
...
-- Softened: longer duration (0.22→0.32) so the Back-easing settle reads as a
-- gentle bounce instead of a snap, with a slightly deeper dip beforehand.
local function showGui()
	...
	mainUIScale.Scale = targetScale * 0.82
	mainOuter.Position = targetPos + UDim2.new(0,0,0,18)
	TweenService:Create(mainUIScale, TweenInfo.new(0.32, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Scale=targetScale}):Play()
	TweenService:Create(mainOuter, TweenInfo.new(0.32, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Position=targetPos}):Play()
end
...
local function hideGui()
	...
	pcall(function()
		...
		-- Softened: longer duration (0.3→0.36) and a gentler Quad curve (was
		-- Quint) so the "suck into the moon" reads as a smooth glide instead
		-- of a hard snap at the end — same direction/target, just eased softer.
		local dur = 0.36
...
buildPage("Settings", function()
	...
	do
		...
		for i, t in ipairs(THEME_LIST) do
			...
			btn.MouseButton1Click:Connect(function()
				if _GH.playThemeTransition then _GH.playThemeTransition() end
				applyTheme(t.id)
				if _GH.autoSave then _GH.autoSave() end
...
footer.BackgroundTransparency = 1; footer.Text = "MOON  •  Speed Bypass"
