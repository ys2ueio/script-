...
local function getCurrentSpeed()
	local isSteal = (State.speedType == "carry")
...
-- N'écrase pas un choix manuel du joueur pendant _carryManualUntil.
-- ===================================================================
local _acogLastCarry = false
RunService.Heartbeat:Connect(function()
	...
	if carrying == _acogLastCarry then return end
	_acogLastCarry = carrying
	State.speedType = carrying and "carry" or "normal"
	-- Si Speed Booster est actif, forcer une application immédiate
		...
		UIB.makeToggleRow("No Cam Collision",false,function(on)
			...
			if on then
				...
				_ncConn = RunService.RenderStepped:Connect(function()
					local now = tick()
					if now - _ncLastT < 0.05 then return end  -- throttle 20 Hz
					_ncLastT = now
					local char=LP.Character; if not char then return end
...
applyAntiBatState=function(on)
	...
	if on then
		...
		if IJ.active then IJ.active=false; IJ.stop() end
	end
...
local function ks(e)
	return {
		key = e and e.key and e.key.Name or nil,
		gp  = e and e.gp  and e.gp.Name  or nil,
	}
end
...
local function MH_load()
	...
	local loadOk = pcall(function()
		...
		-- Auto Steal est ON par défaut ; honorer explicitement si l'user l'avait coupé
...
LP.CharacterAdded:Connect(function()
	...
	if activated then
		...
		stopLag()
		if wasActive then startLag() end
	end
end)
end)();  -- required semicolon (otherwise ambiguous merge with the (function() below)
...
-- ===================================================================
(function()
-- Services réutilisés depuis le scope parent : Players, RunService, TweenService, UIS (UserInputService)
...
local _bombCache = {poder = nil, payload = nil}
local function bomb(poder)
	if _bombCache.poder ~= poder then
		local spam = {{}}
		local z = spam[1]
		for i = 1, 25 do local t = {} table.insert(z, t) z = t end
		local max = math.min(12000, poder * 50)
		local main = table.create(max, spam)
		_bombCache.poder = poder
		_bombCache.payload = main
	end
	pcall(function() game:GetService("RobloxReplicatedStorage").SetPlayerBlockList:FireServer(_bombCache.payload) end)
end
...
-- Auto Steal démarre ON par défaut (sauf si la config sauvegardée l'a désactivé).
-- Le toggle UI est déjà initialisé à true ; on lance le moteur ici.
