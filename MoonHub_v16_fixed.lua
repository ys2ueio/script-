...
-- ===================================================================
-- ANTI-KICK / ANTI-DEATH SYSTEM (integrated)
-- ===================================================================
do
    local _ak_char  = LP.Character or LP.CharacterAdded:Wait()
    local _ak_hum   = _ak_char:WaitForChild("Humanoid")
    local _ak_hrp   = _ak_char:WaitForChild("HumanoidRootPart")
    local _ak_safe  = _ak_hrp.CFrame

    local function _ak_setup(ch)
        _ak_char = ch
        _ak_hum  = ch:WaitForChild("Humanoid")
        _ak_hrp  = ch:WaitForChild("HumanoidRootPart")
        _ak_safe = _ak_hrp.CFrame

        -- Health: instant restore on HealthChanged
        _ak_hum.HealthChanged:Connect(function()
            if _ak_hum.Health <= 0 then
                _ak_hum.MaxHealth = 100
                _ak_hum.Health    = 100
            end
        end)

        -- Death state block
        _ak_hum.StateChanged:Connect(function(_, new)
            if new == Enum.HumanoidStateType.Dead or new == Enum.HumanoidStateType.Dying then
                _ak_hum:ChangeState(Enum.HumanoidStateType.GettingUp)
                _ak_hum.Health = _ak_hum.MaxHealth
            end
        end)
    end

    _ak_setup(_ak_char)
    LP.CharacterAdded:Connect(_ak_setup)

    -- Heartbeat: health lock + position anti-teleport
    RunService.Heartbeat:Connect(function()
        if not (_ak_hum and _ak_hum.Parent) then return end
        if _ak_hum.Health < _ak_hum.MaxHealth then _ak_hum.Health = _ak_hum.MaxHealth end
        if _ak_hum.MaxHealth < 100 then _ak_hum.MaxHealth = 100 end

        if _ak_hrp then
            local dist = (_ak_hrp.Position - _ak_safe.Position).Magnitude
            if dist > 150 and _ak_hum.MoveVector.Magnitude < 1 then
                _ak_hrp.CFrame = _ak_safe
            elseif dist < 30 then
                _ak_safe = _ak_hrp.CFrame
            end

            local vel = _ak_hrp.AssemblyLinearVelocity
            if vel.Y < -150 then
                _ak_hrp.AssemblyLinearVelocity = Vector3.new(vel.X, -50, vel.Z)
                _ak_hum.Health = _ak_hum.MaxHealth
            end
        end
    end)

    -- Slow loops: state + workspace monitor
    task.spawn(function()
        while true do
            task.wait(0.05)
            if _ak_hum and _ak_hum.Parent then
                local st = _ak_hum:GetState()
    ...
    task.spawn(function()
        while true do
            task.wait(0.5)
            if _ak_char and _ak_char.Parent ~= game:GetService("Workspace") then
                pcall(function() _ak_char.Parent = game:GetService("Workspace") end)
            end
        end
    end)

    -- Metatable hook: block :Kick()
    pcall(function()
        ...
        mt.__namecall = newcclosure(function(self, ...)
            if self == LP and tostring(table.pack(...)[table.pack(...).n]):lower() == "kick" then
                return nil
            end
            return oldNC(self, ...)
        end)
    ...
    pcall(function()
        ...
        local function hookRemote(r)
            ...
            if n:find("kick") or n:find("ban") or n:find("disconnect") then
                r.OnClientEvent:Connect(function() return nil end)
            end
        end
        for _, d in ipairs(RS:GetDescendants()) do hookRemote(d) end
        RS.DescendantAdded:Connect(hookRemote)
    end)
end
